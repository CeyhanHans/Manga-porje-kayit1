...
# GOREV5-rapor.md — Performans: OCR Sonuç Önbelleği (Cache)

**Tarih:** 2026-08-30
**Agent:** Echo (GLM-5.2) — bu oturumda SADECE Görev 5 yapıldı, diğer görevlere dokunulmadı.
**Not:** Rapor adı ASCII (`GOREV5`), Görev 2'deki `GÖREV2-rapor.md` Türkçe karakter uyarısına uyum için.

---

## 1. Sorun ve mevcut durum teşhisi

Kullanıcı sayfada yukarı-aşağı scroll ettiğinde / overlay'i kapatıp tekrar açtığında aynı
görsel tekrar tekrar OCR ediliyordu. Mevcut kodda zaten bir `ocrCache` (IIFE içinde `Map`)
vardı ve `processImage` cache hit'te OCR'yı atlıyordu; ANCAK:

1. **TTL yoktu** — entry sonsuza kadar tutuluyordu (görsel değişse bile eski sonuç döner).
2. **Boyut sınırı yoktu** — uzun sayfalarda bellek sınırsız büyür.
3. **`data:` URL'ler cache'leniyordu** — dev base64 anahtarlar bellek şişirir.
4. **`[MangaTR CACHE HIT]` logu yoktu** — cache çalışması gözlemlenemiyordu (başarı kriteri bunu istiyor).
5. **Overlay işareti yoktu** — işlenen görsel DOM düzeyinde işaretlenmiyordu.

## 2. Yapılan değişiklikler

### 2.1 `Desktop\chromium\content.js`

- **Global scope'a (`MANGA_TR_TEXT_FIT` bloğundan hemen sonra, `"use strict"` öncesi) eklendi**
  (`AGENT_GOREV5_BAŞLANGIÇ/BİTİŞ` işaretli — görev talimatındaki "dosyanın başına global
  scope'a ekle" maddesine birebir):
  - `MangaTrOcrCache` (Map), `OCR_CACHE_TTL_MS = 5 dk`, `OCR_CACHE_MAX_SIZE = 50`
  - `getCachedOcr(url)` — TTL aşımında entry'yi siler + `null` döner; geçerliyse
    `[MangaTR CACHE HIT] <url>` loglar + `regions` döner.
  - `setCachedOcr(url, regions)` — `data:image/` anahtarları **kaydetmez**; 50 dolunca
    en eski gireni (ilk anahtarı) atar (FIFO/LRU-vari).
- **Eski `const ocrCache = new Map()` IIFE içinden kaldırıldı** — iki ayrı cache/ölü kod
  olmasın diye; tek cache: `MangaTrOcrCache`.
- **`processImage`:** OCR öncesi `getCachedOcr(key)`; hit'te OCR tamamen atlanır, filtre
  idempotent olduğundan mevcut `filterEnglishOcrBoxes` akışı korunur. Miss'te OCR + `setCachedOcr`.
- **`img.dataset.mangaTrOverlayDone = '1'`** — başarılı render sonrası işaretlenir (görev
  maddesi 4). **Görev 4 çakışma kontrolü:** Görev 4 `dataset.mangaTrProcessed` KULLANMAMIŞ
  (state bazlı çalışıyor); farklı isim seçildi, çakışma yok.
- **`queueImage`:** dataset işaretli görsel tekrar kuyruğa alınmaz (tekrar işlenmez).
- **`clearOverlays`:** kapatma sırasında `mangaTrOverlayDone` işaretleri temizlenir;
  **`MangaTrOcrCache` bilerek korunur** — kullanıcı çeviriyi tekrar açtığında cache hit
  ile OCR'sız anında overlay gelir (görevin ana senaryosu: "aynı görsel ikinci kez
  görüldüğünde OCR yeniden yapılmamalı").

### 2.2 TypeScript `extension-source/src/content/content.ts`

Aynı değişiklikler TS sözdizimiyle (`OcrCacheEntry` tipi, tipli fonksiyonlar). Cache bloğu
dosyanın en başına (namespace dışına, derlenmiş dosyadaki global yerleşimle hizalı) eklendi;
eski `ocrCache` kaldırıldı; `processImage`, `queueImage`, `clearOverlays` aynı şekilde
güncellendi. TS dosyasının güncel kopyası (2026-08-30 23:02 Codex review kopyası, Görev 2
ve 4'ün de üzerinde çalıştığı kopya) düzenlendi.

### 2.3 Dokunulmayanlar

`background.js`, `manifest.json`, OCR dosyaları (`ocr.js`, `ocr-frame.js`,
`ocr-worker.min.js`, `ocr-core/`, `ocr-lang/`), Görev 1/2/3/4 blokları. `popup.*` dokunulmadı.

## 3. Test — 31/31 PASS

**JS (`node --check` + `backup_GOREV5/gorev5-test.js`, gerçek dosyadan çıkarılan kodla):**

| Grup | Testler |
|---|---|
| Yapı S1-S11 | global blok yerleşimi, eski cache kaldırıldı, processImage entegrasyonu, dataset işareti render sonrası, queueImage kontrolü, clearOverlays temizliği, TTL=5dk, MAX=50, data:URL engeli, HIT log formatı, diğer görev blokları bozulmadı |
| Davranış T1-T9 | set→get hit aynı kutular; olmayan anahtar null; **data:URL cache'lenmez**; 6dk eski entry TTL aşımı→null (ve silinir); 4dk eski entry hâlâ geçerli; 60 eklemede boyut=50; ilk giren atılır son kalan olur; **`[MangaTR CACHE HIT]` loglanır**; TTL miss sessizce log üretmez |

**TS (`gorev5-ts-kontrol.py`):** TS1-TS11 hepsi PASS (aynı yapısal şartlar + AGENT_GOREV5 işaretleri).

**Başarı kriteri karşılığı:** "Aynı görsel ikinci kez görüldüğünde `[MangaTR CACHE HIT]`
logu çıkmalı, OCR yeniden yapılmamalı" — T1+T8 kanıtlar; processImage akışı (S3) hit'te
`recognizeImageLocally` çağrısını atlar. Gerçek tarayıcı senaryosu Görev 10'da doğrulanacak.

## 4. Yedekler

`Desktop\chromium\backup_GOREV5\` → `content.js`, `content.ts` (değişiklik öncesi) +
`gorev5-test.js`, `gorev5-ts-kontrol.py` (kanıt scriptleri).

## 5. Notlar / riskler

- Cache anahtarı `imageCacheKey()` ile aynı: `currentSrc|WxH` — görsel içeriği değişirse
  aynı URL'de 5 dakika boyunca eski sonuç dönebilir (TTL bunu sınırlar; görev tanımı URL
  bazlı cache istedi).
- `data:` URL'ler hem görev gereği cache'lenmiyor hem de zaten `pageManagedDataUrl`
  aşamasında短 ömürlüdür.
- Cache hit'te `filterEnglishOcrBoxes` yeniden çalışır — idempotent (zaten filtreli veri),
  yalnızca mikro maliyet.
<!-- AGENT_GOREV5_BİTİŞ -->


---

## 6. Ek — Değişen dosyaların tam içeriği (değişiklik sonrası)

### 6.1 Desktop\chromium\content.js (762 satır)
...
function setCachedOcr(url, regions) {
    if (url.startsWith('data:image/'))
        return; // data: URL'li görseller önbelleğe alınmaz
    if (MangaTrOcrCache.size >= OCR_CACHE_MAX_SIZE) {
        MangaTrOcrCache.delete(MangaTrOcrCache.keys().next().value);
    }
...
(function (MangaTrContent) {
    ...
    .manga-tr-region { position:absolute; display:flex; align-items:center; justify-content:center; box-sizing:border-box; padding:4px; border:0; outline:0; border-radius:5px; background:rgba(255,255,255,.98) !important; color:#111; font-family:Arial,sans-serif; font-weight:700; letter-spacing:0; line-height:1.06; text-align:center; text-shadow:none; white-space:pre-wrap; word-break:break-word; overflow:hidden; }
    /* AGENT_GOREV3_BİTİŞ */
  ...
  `;
    ...
    function queueImage(image) {
        ...
        /* AGENT_GOREV5_BAŞLANGIÇ */
        if (!state || !isMangaImage(image) || !isNearViewport(image) || state.busy || state.queued || state.processed || image.dataset.mangaTrOverlayDone === '1')
            return;
        /* AGENT_GOREV5_BİTİŞ */
        state.queued = true;
        ocrQueue.push(state);
    ...
    function queueVisibleImages() {
        for (const state of overlays.values())
            if (isNearViewport(state.image))
                queueImage(state.image);
    }
    ...
    function clearOverlays() {
        ...
        for (const image of overlays.keys())
            delete image.dataset.mangaTrOverlayDone;
        /* AGENT_GOREV5_BİTİŞ */
        for (const state of overlays.values())
            state.node.remove();
        overlays.clear();
...
### 6.2 extension-source\src\content\content.ts (588 satır)
