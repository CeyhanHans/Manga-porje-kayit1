<!-- AGENT_GOREV6_BAŞLANGIÇ -->
# GOREV6-rapor.md — Build Zinciri Doğrulama ve Tamir

**Tarih:** 2026-08-30/31 gece
**Agent:** Echo (GLM-5.2) — bu görevde SADECE build zinciri; `Desktop\chromium\` kaynak dosyalarına dokunulmadı (yalnız bu rapor + backup klasörü eklendi).
**Çalışma dizini (temiz ortam):** `C:\Users\user\.zcode\workspace\default\gorev6\extension-source\`
**Kaynak (senkron edildi):** `Documents\Codex\2026-08-30\...\Manga-porje-kayit1\extension-source\` (Görev 1-2-4-5'in TS değişikliklerini içeren en güncel kopya)
...
| Adım | Sonuç |
|---|---|
| `npm ci` | ✅ 16 paket, 0 güvenlik açığı |
| `npm run check` (tsc --noEmit) | ✅ **1 düzeltme sonrası** hatasız |
| `npm test` (build + node --test) | ✅ Build OK + **12/12 test PASS** |
| `npm run package` | ✅ `outputs/packages/{chromium,firefox}` sahaya kondu |
| `npm run verify:delivery` | ✅ envanter + dist-hash eşleşmesi + izinler + Firefox CSP + API anahtar taraması PASS |

## 2. Bulunan ve düzeltilen 2 sorun

### Sorun 1 — Kod: `content.ts` erken kapatılmış namespace (tsc TS1128)

```
src/content/content.ts(588,1): error TS1128: Declaration or statement expected.
```

**Kök neden:** `namespace MangaTrContent` dosyanın ortasında (pumpQueue'dan sonra)
kapatılmıştı; `queueImage`, `queueVisibleImages`, `ensureOverlays`, `startPageTranslation`,
`clearOverlays` ve `api.runtime.onMessage` bloğu namespace DIŞINDA kalmış, dosya sonunda
yetim bir `}` kalmıştı. (Muhtemelen Görev 1'in TS eklemesi sırasında bozuldu; kaynak hiç
temiz ortamda derlenmediği için kimse görmedi — görevin teşhisi doğruydu.)

**Düzeltme (mantık değişikliği YOK):** Ortadaki kapanış `}` kaldırıldı → fonksiyonlar tekrar
namespace içine girdi; dosya sonundaki `}` zaten doğru kapanış oldu. `AGENT_GOREV6_BAŞLANGIÇ/BİTİŞ`
işaretli. Bu tek düzeltme `check`'i geçirdi; başka tip hatası çıkmadı (Görev 2/4/5'in
TS değişiklikleri strict modda temiz çıktı).

### Sorun 2 — Eksik varlık: `assets/ocr/eng.traineddata.gz` (10,9 MB)

`build.mjs` satır 69 bu dosyayı bekliyor ama klasör repoda/GitHub kopyasında **hiç yok**
(binary olduğu için repoya hiç girmediği anlaşılıyor). `Desktop\chromium\ocr-lang\eng.traineddata.gz`
(okundu, değiştirilmedi) temiz ortama ve kaynak kopyaya geri kondu. Kod/script değişikliği gerektirmedi.

**Toplam: 2 hata → 1 kod düzeltmesi (3 satır yorum + 1 satır silme) + 1 eksik dosya restorasyonu.**

## 3. ZIP/XPI hash'leri (SHA256)

`package.mjs` ZIP üretmiyor (yalnız klasör sahalar); görev "ZIP/XPI hash'lerini al"
dediği için arşivler PowerShell `Compress-Archive` ile üretildi (build zincirine
dokunulmadı; `outputs/zip/` altına ek teslimat olarak kondu):

```
4F0736815E041A563D8A626DD4D5CD7DEC19D525AC071DDB0A3405717BF5077C  manga-turkce-overlay-0.5.3-chromium.zip   (22.320.551 bayt)
CD2544912C36F0ACBCA14A170CADC953E37760E4545745CBC6D53F669ADC0B83  manga-turkce-overlay-0.5.3-firefox.zip    (22.320.428 bayt)
CD2544912C36F0ACBCA14A170CADC953E37760E4545745CBC6D53F669ADC0B83  manga-turkce-overlay-0.5.3-firefox.xpi   (imzasız XPI = firefox.zip)
```

## 4. Test kanıtı (npm test çıktısı)

```
Built dist/chromium and dist/firefox
✔ visible screenshot crop maps back to natural image coordinates
✔ off-screen image does not produce a screenshot crop
✔ distributed content script contains and executes the shared geometry implementation
✔ distributed content script supports automatic page-wide image overlays
✔ distributed background forwards the page referrer for protected image hosts
✔ distributed Firefox background rewrites protected image request headers
✔ distributed Firefox package declares remote data use and avoids unsafe eval fallbacks
✔ contain letterbox area is centered
✔ cover keeps source mapping proportional
✔ OCR cleanup removes literal escapes and manga OCR debris
✔ nearby OCR lines are grouped but distant lines stay separate
✔ low-confidence OCR garbage is rejected
ℹ tests 12  pass 12  fail 0
```

`verify:delivery` ek kontrolleri de geçti: paket kökleri dist ile hash düzeyinde aynı,
manifest izinleri (storage / DNR / webRequestBlocking) tam, Firefox paketinde güvenli
eval fallback'leri doğrulandı, teslim paketlerinde **gömülü DeepL anahtarı yok**.

## 5. Dokunulan / dokunulmayan dosyalar

| Dosya | Durum |
|---|---|
| `extension-source/src/content/content.ts` | ✏️ namespace kapanış düzeltmesi (AGENT_GOREV6 işaretli) — hem temiz ortamda hem kaynak kopyada |
| `extension-source/assets/ocr/eng.traineddata.gz` | ➕ eksik varlık restore edildi (chromium\ocr-lang'den kopyalandı) |
| `Desktop\chromium\` tümü | 🔒 hiçbir kaynak dosya değiştirilmedi (sadece bu rapor + backup_GOREV6 eklendi) |
| OCR binary/worker, test mantığı, iş mantığı, scripts/*.mjs | 🔒 değiştirilmedi |

## 6. Yedekler

`Desktop\chromium\backup_GOREV6\content.ts` — düzeltme öncesi hali.

## 7. Notlar

- Build çıktısı (`dist/`, `outputs/`) temiz ortam klasöründe üretildi; `Desktop\chromium`
  güncellenmedi (görev yasak + Görev 10 gerçek tarayıcı testinden önce birleştirme kararı
  kote'ye ait).
- `eng.traineddata.gz` 10,9 MB olduğundan GitHub'a push edilmemeli (repoda `.gitignore`
  yok — kaynak sahibinin eklemesi önerilir).
- Çoklu kopya gerçeği raporlandı: derleme, tüm görev agent'larının TS değişikliklerini
  içeren en güncel kopya üzerinden yapıldı ve düzeltme aynı kopyaya geri yazıldı.
<!-- AGENT_GOREV6_BİTİŞ -->


---

## 8. Ek — Değişen dosyanın tam içeriği (değişiklik sonrası)

### 8.1 extension-source/src/content/content.ts (590 satır)
